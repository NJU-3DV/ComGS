...
if __name__ == "__main__":
    ...
    window_size = 11
    B, CH, H, W = 1, 3, 512, 512 
    pm_ssim = SSIM(data_range=1.0, channel=CH, win_size=window_size)
    iterations = 1000
    ...
    for _ in range(iterations):
        ...
        og_ssim_val = ssim(img1_og, img2_og, window_size=window_size)
        mine_ssim_val_same = fused_ssim(img1_mine_same, img2_mine_same, window_size=window_size)
        mine_ssim_val_valid = fused_ssim(img1_mine_valid, img2_mine_valid, window_size=window_size, padding="valid")
        ...
        torch.testing.assert_close(img1_og.grad, img1_mine_same.grad)
        torch.testing.assert_close(img1_mine_same.grad, img1_pm.grad)
    ...
    begin = time.time()
    for _ in range(iterations):
        og_ssim_val = ssim(img1, img2, window_size=window_size)
    ...
    begin = time.time()
    for _ in range(iterations):
        mine_ssim_val = fused_ssim(img1, img2, window_size=window_size)
    torch.cuda.synchronize()
    end = time.time()
    ...
    begin = time.time()
    for _ in range(iterations):
        mine_ssim_val = fused_ssim(img1, img2, window_size=window_size, train=False)
    torch.cuda.synchronize()
    end = time.time()
