...
std::tuple<int, int, torch::Tensor, torch::Tensor,
...
RasterizeGaussiansCUDA(
  ...
  // if bg is a scalar for each channel
  const bool scalar_bg = background.numel() == 3;

  torch::Tensor out_render = torch::empty({0}, float_opts);
  torch::Tensor out_geometry = torch::empty({0}, float_opts);
  torch::Tensor out_material = torch::empty({0}, float_opts);

  if(requires_render)
	out_render = torch::full({NUM_CHAFFELS, H, W}, 0.0, float_opts);
  if(requires_geometry)
	out_geometry = torch::full({GEO_CHAFFELS, H, W}, 0.0, float_opts);
  if(requires_material)
	out_material = torch::full({MAT_CHAFFELS, H, W}, 0.0, float_opts);

  torch::Tensor radii = torch::full({P}, 0, int_opts);
  torch::Tensor accum_max_count = torch::empty({0}, int_opts);
  if(flag_max_count){
	accum_max_count = torch::full({P}, 0, int_opts);
  }
  ...
  if(P != 0)
  {
	...
	auto tup = CudaRasterizer::Rasterizer::forward(
		...
		materials.contiguous().data<float>(),
		tan_fovx,
		tan_fovy,
		prefiltered,
		out_render.contiguous().data<float>(),
		out_geometry.contiguous().data<float>(),
		out_material.contiguous().data<float>(),
...
RasterizeGaussiansBackwardCUDA(
	...
	const float tan_fovx,
	const float tan_fovy,
    const torch::Tensor& dL_dout_render,
	const torch::Tensor& dL_dout_geometry,
	const torch::Tensor& dL_dout_material,
	const torch::Tensor& dc,
	const torch::Tensor& sh,
	const int degree,
	const torch::Tensor& campos,
  ...
  const int P = means3D.size(0);
  int W, H;
  if(dL_dout_render.numel() != 0) {
	H = dL_dout_render.size(1);
	W = dL_dout_render.size(2);
  }
  else {
	H = dL_dout_geometry.size(1);
	W = dL_dout_geometry.size(2);
  }
  ...
  torch::Tensor dL_dbg = torch::zeros_like(background);
  ...
  torch::Tensor dL_dmaterials = torch::zeros({P, 5}, means3D.options());
  ...
  {  
	  CudaRasterizer::Rasterizer::backward(P, degree, M, R, B,
		...
		dL_dout_render.contiguous().data<float>(),
		dL_dout_geometry.contiguous().data<float>(),
		dL_dout_material.contiguous().data<float>(),
		...
		dL_dmaterials.contiguous().data<float>(),
		debug,
		requires_render,
		requires_geometry,
		requires_material);
