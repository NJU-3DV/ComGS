	...
	while ((t_start < T_SCENE_MAX) && (T > params.transmittance_min)){
		...
		for (int i = 0; i < MAX_BUFFER_SIZE; ++i) {
			...
			if (primIdx == -1) {
				...
				glm::vec3 d_g = SinvR * ray_d;
				float d = - o_g.z / d_g.z;
				float det_x = o_g.x + d_g.x * d;
				float det_y = o_g.y + d_g.y * d;
				float power = det_x * det_x + det_y * det_y;
				float G = __expf(-0.5f * power);
				float alpha = min(0.99f, o * G);
				...
				float dL_dpower = dL_dG * (-0.5) * power;
				float dL_ddet_x = 2 * dL_dpower * det_x;
				float dL_ddet_y = 2 * dL_dpower * det_y;

				glm::vec3 dL_dog = {0.f, 0.f, 0.f};
				dL_dog.x = dL_ddet_x;
				dL_dog.y = dL_ddet_y;
				dL_dog.z -= (d_g.x / d_g.z) * dL_ddet_x;
				dL_dog.z -= (d_g.y / d_g.z) * dL_ddet_y;

				glm::vec3 dL_ddg = {0.f, 0.f, 0.f};
				dL_ddg.x = d * dL_ddet_x;
				dL_ddg.y = d * dL_ddet_y;
				dL_ddg.z -= (d_g.x / d_g.z) * d * dL_ddet_x;
				dL_ddg.z -= (d_g.y / d_g.z) * d * dL_ddet_y;

				glm::mat3x3 dL_dog_dSinvR = glm::outerProduct(dL_dog, ray_o - mean3D);
				glm::mat3x3 dL_ddg_dSinvR = glm::outerProduct(dL_ddg, ray_d);
				glm::mat3x3 dL_dSinvR = dL_dog_dSinvR + dL_ddg_dSinvR;

				glm::vec3 dL_dmean3D = - glm::transpose(SinvR) * dL_dog;
